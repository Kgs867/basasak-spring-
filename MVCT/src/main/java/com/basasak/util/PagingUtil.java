...
public class PagingUtil {
	 ...
	 * pageUrl : 호출 페이지 url(=이전,다음의 이동 버튼)
	...
	public PagingUtil(String keyField, String keyWord, int currentPage, int totalCount, int blockCount,
		...
		// 전체 페이지 수, ceil(무조건 올림)
		int totalPage = (int) Math.ceil((double) totalCount / blockCount);
		if (totalPage == 0) { // 총페이지수가 없으면 무조건 1페이지
			totalPage = 1;
		}
		...
		// StringBuffer : 문자열 + 문자열은 append메서드를 이용
		if (currentPage > blockPage) {
			if(keyWord==null){//검색 미사용시
				pagingHtml.append("<a href="+pageUrl+"?pageNum="+ (startPage - 1) + addKey +">");
			}else{ // 검색어를 입력했다면 keyField(검색분야), keyWord(검색어)를 매개변수로 전달
		...
		for (int i = startPage; i <= endPage; i++) {
			if (i > totalPage) { // 현재페이지가 총페이지수를 넘어가면 더이상 출력하지 않도록
=======
//페이징처리해주는 클래스
public class PagingUtil {
